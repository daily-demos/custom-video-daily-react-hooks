...
export default function App() {
  ...
  const startHairCheck = useCallback(async (url) => {
    ...
    await newCallObject.preAuth({ url, token: process.env.REACT_APP_TOKEN }); // add a meeting token here if your room is private
  ...
  const joinCall = useCallback(() => {
    callObject.join({ url: roomUrl, token: process.env.REACT_APP_TOKEN });
  }, [callObject, roomUrl]);
